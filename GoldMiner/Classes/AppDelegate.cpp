...
bool AppDelegate::applicationDidFinishLaunching() {
	...
	char totalFrames2 = 4;
	char frameName2[20];
	Animation* helicpAnimation = Animation::create();

	for (int i = 0; i < totalFrames2; i++)
	{
		sprintf(frameName2, "helicopter-%d.png", i);
		helicpAnimation->addSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(frameName2));
	}
	helicpAnimation->setDelayPerUnit(0.1);
	AnimationCache::getInstance()->addAnimation(helicpAnimation, "helicpAnimation");

	// load game resource
	SpriteFrameCache::getInstance()->addSpriteFramesWithFile("level-sheet.plist");
	char totalFrames3 = 7;
	char frameName3[20];
	Animation* goldAnimation = Animation::create();

	for (int i = 0; i < totalFrames3; i++)
	{
		sprintf(frameName3, "gold-0-%d.png", i);
		goldAnimation->addSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(frameName3));
	}
	goldAnimation->setDelayPerUnit(0.1);
	AnimationCache::getInstance()->addAnimation(goldAnimation, "goldAnimation");
